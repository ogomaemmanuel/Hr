  ...
  <div>
    <template v-if="invoiceLoaded">
      ...
      <div class="mt-3 pb-5 flex justify-center">
        <div class="mr-2">
          <button @click="updateInvoice" class="button is-primary is-rounded">Save and Send</button>
        </div>
        <div class="ml-2">
...
export default {
  ...
  methods: {
    ...
    getInvoiceById() {
      axios.get(`/api/invoices/${this.invoiceId}`).then(resp => {
        this.invoice = resp.data;
        this.selectClient = this.invoice.client;
        this.selectClient.email = this.invoice.client.user.email;
    ...
    getTaxes() {
      axios.get("/api/taxes/all").then(resp => {
        ...
        console.log("Error fetching taxes  data in invoiceCreateForm")
      })
    }
  }
